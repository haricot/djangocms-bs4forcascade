...
from .utils import BS4_BREAKPOINT_KEYS, reduce_breakpoints, get_picture_elements
...
class BootstrapPicturePlugin(ImageAnnotationMixin, LinkPluginBase):
    ...
    responsive_heights = GlossaryField(
        MultipleCascadingSizeWidget(BS4_BREAKPOINT_KEYS, allowed_units=['px', '%'], required=False),
    ...
    responsive_zoom = GlossaryField(
        MultipleCascadingSizeWidget(BS4_BREAKPOINT_KEYS, allowed_units=['%'], required=False),
